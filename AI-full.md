# AI Usage — full history

> Condensed version: [AI.md](AI.md). This file keeps the complete per-increment
> record: full prompt history, every design question with the options considered,
> and the artifacts that guided each increment.
...
4. Claude implements the code and tests, runs `./mvnw test`; I review and commit.
...
| Exception for unknown id | `IllegalArgumentException` vs. custom `MatchNotFoundException` | **`IllegalArgumentException`** — consistent with the no-custom-hierarchy convention (later revised, see below) |
...
## Documentation revision (2026-07-09)

> *(translated from Polish)* "Shorten AI.md and README.md — right now it's too
> much text for a recruiter to read IMO. […] Additionally, restore the old AI.md
> into e.g. AI-full.md and note in AI.md that the full version lives there."

Result: `AI.md` and `README.md` condensed to a 2–3 minute read; this file keeps
the complete record.
